...
import { getModulesForBook, getLessonsForModule, getBook } from '@/content';
...
function deriveModuleMastery(lessons: LessonDisplayData[]): MasteryLevel {
...
function useModuleDisplayData(bookId: string): {
  ...
  const moduleData = useMemo(() => {
    const bookModules = getModulesForBook(bookId);
    ...
    return bookModules
      ...
      .map((mod) => {
        const contentLessons: Lesson[] = getLessonsForModule(mod.id, bookId);
...
function ModuleAccordion({ module, index, bookId }: { module: ModuleDisplayData; index: number; bookId: string }) {
...
function LessonRow({
  ...
  if (isClickable) {
    return (
      <Link
        href={`/books/${bookId}/learn/${moduleId}/${lesson.id}`}
...
export default function BookLearnPage() {
  ...
  const { moduleData, isLoading } = useModuleDisplayData(bookId);
        ...
        <div className="container-wide py-8 sm:py-12">
          ...
          <div className="flex flex-col gap-3">
            {moduleData.map((mod, i) => (
              <ModuleAccordion key={mod.id} module={mod} index={i} bookId={bookId} />
            ))}
          </div>
        </div>
      </PageTransition>
