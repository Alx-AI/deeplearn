...
import { MASTERY_LABELS, MASTERY_COLORS } from '@/lib/srs/mastery';
import {
  getModulesForBook,
  getTotalCardCount,
  getTotalLessonCount,
...
export default function BookProgressPage() {
  ...
  const { getProgress } = useLessonProgress();
  const { todayCount, totalCount: totalReviewCount } = useReviewStats();
  ...
  const bookModules = useMemo(() => getModulesForBook(bookId), [bookId]);

  const moduleData: ModuleData[] = useMemo(() => {
    return bookModules.map((mod) => {
  ...
  const totalLessons = getTotalLessonCount(bookId);
