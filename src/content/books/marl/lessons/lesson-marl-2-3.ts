...
$$G = r_0 + r_1 + \\ldots + r_{T-1}$$

Because MDPs are stochastic -- actions can lead to different outcomes -- the agent cannot guarantee a specific return in every episode. Instead, it maximizes the **expected return** $\\mathbb{E}_{\\pi}[r_0 + r_1 + \\ldots + r_{T-1}]$, where the expectation is taken over the initial state distribution $\\mu$, the policy $\\pi$, and the transition function $T$.
...
$$G_t = r_t + \\gamma \\cdot r_{t+1} + \\gamma^2 \\cdot r_{t+2} + \\ldots = \\sum_{k=0}^{\\infty} \\gamma^k \\cdot r_{t+k}$$

The discount factor ensures this infinite sum is bounded. Specifically, if rewards lie in a finite range $[r_{\\min}, r_{\\max}]$, the discounted return satisfies:

$$G_t \\leq \\frac{r_{\\max}}{1 - \\gamma}$$

This upper bound comes from the closed-form of the geometric series $\\sum_{k=0}^{\\infty} \\gamma^k = \\frac{1}{1 - \\gamma}$. So even in a never-ending interaction, the discounted return is always a finite, well-defined quantity.

One crucial notational point: the discounted return has a **recursive structure** that will be central to everything that follows. We can write $G_t = r_t + \\gamma \\cdot G_{t+1}$. This recursive decomposition is the seed from which Bellman equations grow, and it is the reason bootstrapping methods work.
`,
...
$$V^{\\pi^*}(s) \\geq V^{\\pi'}(s) \\quad \\text{for all policies } \\pi' \\text{ and all states } s \\in \\mathcal{S}$$
...
- The discounted return $G_t = \\sum \\gamma^k \\cdot r_{t+k}$ ensures finite, well-defined cumulative rewards even in infinite-horizon tasks.
- The recursive structure $G_t = r_t + \\gamma \\cdot G_{t+1}$ is the foundation for Bellman equations and bootstrapping methods.
