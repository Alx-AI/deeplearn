...
**Weakly homogeneous agents:** for any joint policy $\\pi = (\\pi_1, \\pi_2, \\ldots, \\pi_n)$ and any permutation $\\sigma$ of agents, swapping which agent runs which policy does not change the expected returns:

$$U_i(\\pi) = U_{\\sigma(i)}(\\langle \\pi_{\\sigma(1)}, \\pi_{\\sigma(2)}, \\ldots, \\pi_{\\sigma(n)} \\rangle) \\quad \\forall i$$
...
$$\\theta_{\\text{shared}} = \\theta_1 = \\theta_2 = \\ldots = \\theta_n \\quad (\\text{and/or } \\phi_{\\text{shared}} = \\phi_1 = \\ldots = \\phi_n)$$
...
$$L(\\phi_i) = [\\text{on-policy term for agent } i] - \\lambda \\sum_{k \\neq i} \\frac{\\pi(a_k \\mid h_k \\;; \\phi_i)}{\\pi(a_k \\mid h_k \\;; \\phi_k)} \\cdot \\text{Adv}_k \\cdot \\log \\pi(a_k \\mid h_k \\;; \\phi_i)$$
