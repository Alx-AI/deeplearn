...
The algorithms we study in later chapters are general: they learn policies over the full action set $A_i = X_i \\times M_i$, treating communication actions on equal footing with environment actions. Some specialised algorithms (such as CommNet and DIAL, discussed later) add architectural biases that make communication learning easier, but the fundamental principle is the same: communication is just another action whose value must be discovered through experience.`,
