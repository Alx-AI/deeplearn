...
- **Backpropagation:** propagates the evaluation $u$ backward through the visited nodes, updating Q-values. For games where all rewards are zero until the terminal state, we can directly average: $Q(s, a) \\leftarrow Q(s, a) + \\frac{1}{N(s,a)} (u - Q(s, a))$.

- **BestAction:** after all $k$ simulations, selects an action for the current state, typically the most-visited action: $a_t \\in \\arg\\max_a N(s_t, a)$.
...
$$\\pi_1(s^t_1) \\to a^t \\to \\pi_1(\\psi(s^{t+1}_2)) \\to a^{t+1} \\to \\pi_1(s^{t+2}_1) \\to a^{t+2} \\to \\ldots$$
...
The neural network $f(s \\;; \\theta)$ takes a game state as input and outputs two things:
...
$$L(\\theta) = (z - u)^2 - \\pi^T \\log p + c \\|\\theta\\|^2$$
...
$$a = \\arg\\max_a \\left[ Q(s, a) + C(s) \\cdot P(s, a) \\cdot \\frac{\\sqrt{N(s)}}{1 + N(s, a)} \\right]$$
