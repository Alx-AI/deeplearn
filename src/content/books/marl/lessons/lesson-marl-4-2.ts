  ...
  sections: [
    {
      ...
      content: `Most solution concepts in game theory can be expressed compactly using one elegant idea: the **best response**. Given policies for all agents except agent $i$, denoted $\\pi_{-i} = (\\pi_1, \\ldots, \\pi_{i-1}, \\pi_{i+1}, \\ldots, \\pi_n)$, a best response for agent $i$ is any policy $\\pi_i$ that maximizes $i$'s expected return when played against $\\pi_{-i}$. Formally:
...
A minimax solution can equivalently be understood as each agent using a best-response policy to the other agent's policy: $\\pi_i \\in BR_i(\\pi_j)$ and $\\pi_j \\in BR_j(\\pi_i)$. This mutual-best-response interpretation will generalize to Nash equilibrium in the next lesson.`,
