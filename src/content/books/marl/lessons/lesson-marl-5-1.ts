...
**Data:** A set of $z$ histories, written $D_z = \\{h^e_t \\mid e = 1, \\ldots, z\\}$. Each history $h^e_t$ was produced by whatever joint policy $\\pi^e$ the agents used during episode $e$. Histories may vary in length and may or may not end in terminal states. Typically $D_z$ includes the history from the current ongoing episode plus all histories from previous episodes.
...
`,
    ...
    },
    {
      ...
      content: `
Even in single-agent RL, learning creates a subtle difficulty known as the **moving target problem**. When an agent uses temporal-difference methods to learn action values $Q(s, a)$, the update target -- something like $r + \\gamma \\cdot Q(s', a')$ -- depends on value estimates that are themselves changing as the policy improves. The target is literally moving as you try to hit it.

In MARL, this problem is dramatically **amplified**. From the perspective of any individual agent $i$, the other agents' policies $\\pi_j$ (for all $j \\neq i$) become part of the apparent environment dynamics. Specifically, the effective transition function that agent $i$ perceives is:

$$T_i(s' \\mid s, a_i) \\propto \\sum_{a_{-i}} T(s' \\mid s, \\langle a_i, a_{-i} \\rangle) \\prod_{j \\neq i} \\pi_j(a_j \\mid s)$$
