...
**Learning rate (alpha).** This is arguably the single most important hyperparameter. Too high, and the policy oscillates or collapses; too low, and training stalls. Typical values range from 10^{-4} to 10^{-3}. A2C and PPO often work well with alpha = 3 * 10^{-4}. The textbook's REINFORCE experiments used alpha = 10^{-3}. Learning rate schedules (e.g. linear decay) can help, but a good constant rate is a solid starting point.
...
**Advantage normalisation.** The raw advantage estimates can have large magnitude and high variance across a batch. A common trick is to normalise advantages within each mini-batch to have zero mean and unit variance: Adv_norm = (Adv - mean(Adv)) / (std(Adv) + eps). This does not change which actions are better than average, but it controls the scale of the gradient, making training more stable.

**Gradient clipping.** Even with careful hyperparameter tuning, occasional large gradients can destabilise training. **Gradient norm clipping** caps the global norm of the gradient vector to a maximum value (e.g. 0.5 or 1.0): if ||g|| > max_norm, scale g to g * (max_norm / ||g||). This prevents catastrophic parameter updates from outlier batches.
...
- The learning rate is the most important hyperparameter; 3e-4 is a common starting point for A2C and PPO. Discount factor gamma=0.99 and N-step N=5 are standard defaults.
