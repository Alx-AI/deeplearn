...
An **observation** $o_t$ is what the agent actually perceives. In a partially observable environment, $o_t$ may omit important information. A robot with a forward-facing camera, for example, cannot see what is behind it. A sensor might measure position but not velocity. The observation is generated by an observation function $O(o \\mid s)$, which maps states to (possibly stochastic) observations.

The key problem with partial observability is that the current observation alone may be insufficient for optimal decision-making. Two different states might produce the same observation, but require different actions. To recover the necessary information, the agent needs to consider its **history** of observations: $h_t = (o_0, o_1, \\ldots, o_t)$. The full history is a sufficient statistic for the underlying state -- in principle, the agent can extract all available information about the true state from the sequence of everything it has observed.

When we write $\\pi(a \\mid s; \\phi)$ in a fully observable setting, the input to the policy network is the state $s$. In a partially observable setting, the input should instead be the history $h_t$: $\\pi(a \\mid h_t; \\phi)$. The same applies to value functions: $V(h_t; \\theta)$ rather than $V(s; \\theta)$. This seemingly small change has major architectural implications, because the history is a variable-length sequence that grows with every time step.
`,
...
$$h_t = \\text{RNN}(\\text{encode}(o_t),\\, h_{t-1};\\, \\theta_{\\text{rnn}})$$

The updated hidden state $h_t$ is then passed to the policy and value heads:

$$\\pi(a \\mid h_t; \\phi) \\quad \\text{and} \\quad V(h_t; \\theta_v)$$
...
In the **fully decentralised** setting, each agent's policy depends only on its own observation history: $\\pi_i(a \\mid h_t^i; \\phi_i)$. This is the only option at execution time in many real-world deployments, where communication is limited or impossible. The policy must be self-contained.
...
For value-based methods like DQN variants, CTDE might mean training a centralised Q-function $Q(s, a_1, \\ldots, a_n; \\theta)$ that takes the global state and all agents' actions, while each agent still selects its action based on its local observation. For actor-critic methods, the critic $V(s; \\theta)$ or $Q(s, a_1, \\ldots, a_n; \\theta)$ uses global information, while each actor $\\pi_i(a \\mid o_t^i; \\phi_i)$ uses only local observations.
...
- The state is the complete environment description; an observation is the agent's partial view. In partially observable environments, the agent must condition on its history of observations $h_t = (o_0, \\ldots, o_t)$.
