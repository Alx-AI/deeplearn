  ...
  sections: [
    {
      ...
      content: `
DQN and other value-based methods learn a value function and derive a policy from it (e.g. epsilon-greedy). **Policy gradient** methods take a fundamentally different approach: they represent the policy itself as a parameterised function pi(a | s; phi) and optimise its parameters directly to maximise expected returns.
...
pi(a | s; phi) = exp(l(s, a; phi)) / sum_{a'} exp(l(s, a'; phi))
...
nabla_phi J(phi) = E_{s ~ Pr(.|pi), a ~ pi} [ Q^pi(s, a) * nabla_phi log pi(a | s; phi) ]
...
u_t = sum_{tau=t}^{T-1} gamma^{tau-t} * r_tau

The REINFORCE loss for a single episode of length T is:

L(phi) = -(1/T) * sum_{t=0}^{T-1} u_t * log pi(a_t | s_t; phi)
...
nabla_phi J(phi) = E_pi [ (Q^pi(s, a) - b(s)) * nabla_phi log pi(a | s; phi) ]
...
E_pi [ b(s) * nabla_phi log pi(a | s; phi) ] = sum_s Pr(s|pi) * b(s) * nabla_phi (sum_a pi(a|s;phi)) = sum_s Pr(s|pi) * b(s) * nabla_phi(1) = 0
...
The most common choice of baseline is a learned **state-value function** V(s; theta). The REINFORCE loss with a value-function baseline becomes:

L(phi) = -(1/T) * sum_{t=0}^{T-1} (u_t - V(s_t; theta)) * log pi(a_t | s_t; phi)

The value function is trained alongside the policy to minimise its own MSE loss:

L(theta) = (1/T) * sum_{t=0}^{T-1} (u_t - V(s_t; theta))^2

The intuition is clear: the term (u_t - V(s_t; theta)) measures how much better (or worse) the actual return was compared to the expected return. Actions that produced above-average returns get reinforced; below-average actions get discouraged. Without the baseline, even mediocre actions might receive a large positive gradient simply because the environment gives positive rewards everywhere.
...
- The policy gradient theorem provides a gradient expression proportional to Q^pi(s,a) * nabla log pi(a|s;phi), computable from on-policy experience without knowing the transition function.
