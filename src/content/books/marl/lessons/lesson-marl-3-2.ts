...
Given a normal-form game $\\Gamma = (I, \\{A_i\\}, \\{R_i\\})$, a repeated normal-form game plays the same game $\\Gamma$ over $T$ time steps $t = 0, 1, \\ldots, T-1$, where $T$ can be finite or infinite. At each time step $t$, every agent $i$ samples an action $a_i^t$ from its policy $\\pi_i(a_i^t \\mid h^t)$, which is now conditioned on the **joint-action history** $h^t = (a^0, a^1, \\ldots, a^{t-1})$ -- the record of all joint actions from every previous round. (At $t = 0$ the history is empty.) After all agents act, each agent $i$ receives reward $r_i^t = R_i(a^t)$, identical to the per-round reward defined by the underlying normal-form game.
