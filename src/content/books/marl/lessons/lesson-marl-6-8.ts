...
$$\\alpha_{k+1} = \\alpha_k + l^k_i \\, \\kappa \\, \\frac{\\partial U_i}{\\partial \\alpha}$$
$$\\beta_{k+1} = \\beta_k + l^k_j \\, \\kappa \\, \\frac{\\partial U_j}{\\partial \\beta}$$

where $l^k_i$ and $l^k_j$ are chosen as:

$$l^k_i = l_{\\min} \\;\\text{ if }\\; U_i(\\alpha_k, \\beta_k) > U_i(\\alpha_e, \\beta_k) \\;\\text{ (winning)}$$
$$l^k_i = l_{\\max} \\;\\text{ otherwise (losing)}$$
...
This modified algorithm is called **WoLF-IGA**. Its dynamical system has the same structure as IGA but with time-varying learning rates in the matrix $F(t)$. The magic happens in the problematic Case 3 (imaginary eigenvalues, center in the unit square): the piecewise-elliptical trajectories now **spiral inward** toward the center point. In each of four quadrants around the center, the ellipse tightens by a factor of $\\sqrt{l_{\\min} / l_{\\max}} < 1$. Since $l_{\\min} < l_{\\max}$, this factor is strictly less than 1, guaranteeing convergence to the Nash equilibrium.
`,
...
$$Q(s_t, a^t_i) \\leftarrow Q(s_t, a^t_i) + \\alpha \\bigl[r^t_i + \\gamma \\max_{a'_i} Q(s_{t+1}, a'_i) - Q(s_t, a^t_i)\\bigr]$$

**2. Average policy tracking.** Maintain an average policy $\\bar{\\pi}_i(a_i \\mid s)$ that averages over all past policies:

$$C(s) \\leftarrow C(s) + 1$$
$$\\bar{\\pi}_i(a_i \\mid s) \\leftarrow \\bar{\\pi}_i(a_i \\mid s) + \\frac{1}{C(s)} \\bigl[\\pi_i(a_i \\mid s) - \\bar{\\pi}_i(a_i \\mid s)\\bigr]$$
...
$$\\delta = l_w \\;\\text{ if }\\; \\sum_{a_i} \\pi_i(a_i\\mid s) \\, Q(s, a_i) > \\sum_{a_i} \\bar{\\pi}_i(a_i\\mid s) \\, Q(s, a_i) \\;\\text{ (winning)}$$
$$\\delta = l_l \\;\\text{ otherwise (losing)}$$
...
A key design choice in WoLF-PHC: it replaces the Nash equilibrium policy (used in WoLF-IGA to determine winning/losing) with the **average policy** $\\bar{\\pi}_i$. This is essential because in a general stochastic game, the Nash equilibrium is not known a priori. The average policy is a practical substitute that tends toward equilibrium over time.
...
- The WoLF principle uses variable learning rates: learn fast when losing ($l_{\\max}$), learn slowly when winning ($l_{\\min}$), breaking the symmetric cycling of IGA.
- WoLF-IGA provably converges to Nash equilibrium in all two-agent, two-action games by spiraling inward with a tightening factor of $\\sqrt{l_{\\min}/l_{\\max}}$.
