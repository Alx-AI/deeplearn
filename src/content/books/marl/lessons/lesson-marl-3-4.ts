...
2. An **observation function** O_i : A x S x O_i -> [0,1] that assigns, for each joint action a and resulting state s, a probability distribution over agent i's possible observations.

At each time step t, the game is in state s_t. Each agent i receives an observation o_i^t drawn from O_i(o_i^t | a^{t-1}, s_t). The agent then selects an action a_i^t according to its policy pi_i(a_i^t | h_i^t), where h_i^t = (o_i^0, ..., o_i^t) is agent i's **observation history** -- the only information it has access to. Note that different agents generally have different observation histories, meaning they reason from *different information*.
...
b_i^{t+1}(s') is proportional to the sum over s of b_i^t(s) * T(s' | s, a_i^t) * O_i(o_i^{t+1} | a_i^t, s').
...
- **Limited view region:** o_i^t = (s_bar_t, a_bar_t), where s_bar and a_bar are subsets of the state and joint action within the agent's local vision field.
- **Noisy observations:** O_i assigns nonzero probability to multiple observations for a given state, modelling sensor noise or unreliable communication channels.
