...
If we try the single-agent approach, we need a central controller that chooses a joint action -- one action for *each* of the three robots at every time step. That means the action space is $6 \\times 6 \\times 6 = 216$ possible joint actions. Even in this toy scenario, most standard single-agent RL algorithms struggle with action spaces that large. Scale up to 100 warehouse robots, and you get $6^{100}$ possible joint actions -- a number with 78 digits. Clearly, centralized control does not scale.
