...
d_i = R(s, <a_i, a_{-i}>) - R(s, <tilde{a}_i, a_{-i}>)

This "counterfactual" tells agent i how much its specific action mattered. But computing it requires (1) choosing a default action tilde{a}_i and (2) access to the reward function. The **aristocrat utility** avoids choosing a specific default by averaging over agent i's current policy:

d_i = R(s, <a_i, a_{-i}>) - E_{a'_i ~ pi_i}[ R(s, <a'_i, a_{-i}>) ]
...
Adv_i(h_i, z, a) = Q(h_i, z, a ; theta) - sum_{a'_i in A_i} pi(a'_i | h_i ; phi_i) * Q(h_i, z, <a'_i, a_{-i}> ; theta)
...
pi_{-i}^+ in argmax_{pi_{-i}} U_i(pi_i, pi_{-i})
...
L(phi_i) = -E[ log pi(a_i | h_i ; phi_i) * (Q^{pi+}(h_i, z, <a_i, a_{-i}> ; theta_q) - V^{pi+}(h_i, z ; theta_v)) ]
