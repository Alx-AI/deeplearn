...
The core idea: agent $i$ maintains a single joint-action Q-function $Q_i(s, a)$ (unlike JAL-GT, which maintains Q-functions for **all** agents). Agent $i$ also maintains models $\\hat{\\pi}_j(a_j \\mid s)$ for each other agent $j$. The model and Q-values together let agent $i$ compute the expected value of any action $a_i$ in state $s$:

$$AV_i(s, a_i) = \\sum_{a_{-i}} Q_i(s, (a_i, a_{-i})) \\prod_{j \\neq i} \\hat{\\pi}_j(a_j \\mid s)$$
...
$$a_i = \\arg\\max_{a_i} AV_i(s, a_i)$$

The TD update uses this same $AV$ function for the bootstrap target:

$$Q_i(s_t, a_t) \\leftarrow Q_i(s_t, a_t) + \\alpha \\bigl[r^t_i + \\gamma \\max_{a'_i} AV_i(s_{t+1}, a'_i) - Q_i(s_t, a_t)\\bigr]$$
