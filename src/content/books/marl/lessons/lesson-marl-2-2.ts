...
1. $\\mathcal{S}$ -- a finite set of states, with a subset of terminal states $\\bar{S} \\subseteq \\mathcal{S}$.
2. $\\mathcal{A}$ -- a finite set of actions available to the agent.
3. $T : \\mathcal{S} \\times \\mathcal{A} \\times \\mathcal{S} \\to [0, 1]$ -- the **state transition probability function**. $T(s, a, s')$ gives the probability of transitioning to state $s'$ when the agent takes action $a$ in state $s$. For every state-action pair, these probabilities must sum to 1 over all possible next states: $\\sum_{s'} T(s, a, s') = 1$.
4. $R : \\mathcal{S} \\times \\mathcal{A} \\times \\mathcal{S} \\to \\mathbb{R}$ -- the **reward function**. $R(s, a, s')$ returns a scalar reward received when the agent takes action $a$ in state $s$ and the environment transitions to $s'$.
5. $\\mu : \\mathcal{S} \\to [0, 1]$ -- the **initial state distribution**, specifying the probability of starting in each state. Terminal states have zero probability under $\\mu$.

An MDP episode unfolds as follows. An initial state $s_0$ is sampled from $\\mu$. At each time step $t$, the agent observes state $s_t$ and selects an action $a_t$ according to its policy $\\pi(a_t \\mid s_t)$. The environment then transitions to state $s_{t+1}$ with probability $T(s_{t+1} \\mid s_t, a_t)$, and the agent receives reward $r_t = R(s_t, a_t, s_{t+1})$. This loop repeats until a terminal state is reached, a maximum number of time steps $T$ elapses, or (in non-terminating MDPs) indefinitely.

A concrete example helps ground this abstraction. Consider a **Mars Rover MDP** in which a rover must return to its base station after collecting samples. The state set $\\mathcal{S}$ includes Start, Site A, Site B, Base, Immobile, and Destroyed. From each non-terminal state, the rover can choose between actions "left" and "right." Each action leads to a successor state with an associated transition probability and reward -- for instance, going right from Start reaches Base with probability 0.5 (reward +10) but also risks destruction with probability 0.5 (reward -10). The transition function $T$ and reward function $R$ together encode the full dynamics of this decision problem.
`,
...
$$\\Pr(s_{t+1}, r_t \\mid s_t, a_t, s_{t-1}, a_{t-1}, \\ldots, s_0, a_0) = \\Pr(s_{t+1}, r_t \\mid s_t, a_t)$$
...
In a **deterministic MDP**, every state-action pair leads to exactly one successor state with probability 1. If the rover chooses "right" in state Site A, it always ends up in the same next state. Formally, for every $(s, a)$, there exists a unique $s'$ such that $T(s' \\mid s, a) = 1$ and $T(s'' \\mid s, a) = 0$ for all $s'' \\neq s'$. Deterministic MDPs are simpler to analyze because there is no uncertainty in the transitions -- the only source of variability is the agent's own (possibly stochastic) policy.
...
- An MDP is defined by a tuple $(\\mathcal{S}, \\mathcal{A}, T, R, \\mu)$: states, actions, transition probabilities, rewards, and an initial state distribution.
