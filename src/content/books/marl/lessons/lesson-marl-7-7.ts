...
The actor is a parameterised policy $\\pi(a \\mid s; \\phi)$ -- exactly as in REINFORCE. The critic is a state-value function $V(s; \\theta)$ that estimates the expected return from state $s$ under the current policy. Together they form a two-network architecture, each with its own parameters and loss function.
...
$$\\mathbb{E}[u_t \\mid s_t] \\approx r_t + \\gamma V(s_{t+1}; \\theta)$$
...
$$G_t^{(N)} = \\sum_{\\tau=0}^{N-1} \\gamma^\\tau r_{t+\\tau} + \\gamma^N V(s_{t+N}; \\theta)$$
...
$$A(s_t, a_t) = r_t + \\gamma V(s_{t+1}; \\theta) - V(s_t; \\theta)$$
...
The key insight is **importance sampling**. If the data was collected by a behaviour policy $\\pi_\\beta$, we can adjust the gradient using the ratio:

$$\\rho(s, a) = \\frac{\\pi(a \\mid s; \\phi)}{\\pi_\\beta(a \\mid s)}$$
...
$$L(\\phi) = -\\min\\left( \\rho \\cdot A, \\, \\text{clip}(\\rho,\\, 1 - \\epsilon,\\, 1 + \\epsilon) \\cdot A \\right)$$
...
In practice, PPO collects a batch of experience, stores the log-probabilities under the behaviour policy $\\pi_\\beta$, and then runs several optimisation epochs (commonly $N_e = 4$) on the same batch, recomputing $\\rho$ at each epoch. The clipping ensures that even after several epochs, the policy does not drift too far.
...
- The advantage function $A(s,a) = Q(s,a) - V(s)$ measures how much better an action is than average. In A2C, it is estimated from the TD error: $r + \\gamma V(s') - V(s)$.
