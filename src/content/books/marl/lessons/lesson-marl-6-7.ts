...
R_i = | r_{1,1}  r_{1,2} |    R_j = | c_{1,1}  c_{1,2} |
      | r_{2,1}  r_{2,2} |          | c_{2,1}  c_{2,2} |
...
alpha_{k+1} = alpha_k + kappa * (dU_i/dalpha)
beta_{k+1} = beta_k + kappa * (dU_j/dbeta)

where the partial derivatives have clean closed forms:

dU_i/dalpha = beta * u + (r_{1,2} - r_{2,2})
dU_j/dbeta = alpha * u' + (c_{2,1} - c_{2,2})
...
[dalpha/dt]   [0    u ] [alpha]   [r_{1,2} - r_{2,2}]
[dbeta/dt ] = [u'   0 ] [beta ] + [c_{2,1} - c_{2,2}]

The off-diagonal matrix **F** with entries u and u' determines everything about the trajectory. The eigenvalues of F satisfy lambda^2 = u * u', leading to three qualitatively different behaviors:
...
**Case 2: F has purely real eigenvalues (u * u' > 0).** The trajectory diverges away from the center point. This happens in common-reward and some general-sum games, but **never** in zero-sum games (since u' = -u implies u * u' <= 0).

**Case 3: F has purely imaginary eigenvalues (u * u' < 0).** The trajectory follows **ellipses** around the center point. This is the iconic cycling behavior, occurring in zero-sum and some general-sum games. The center point is (alpha*, beta*) = ((c_{2,2} - c_{2,1})/u', (r_{2,2} - r_{1,2})/u).

The elliptical cycling in Case 3 is the most interesting and problematic. The joint policy orbits forever without converging. This is a concrete illustration of the **non-stationarity** challenge in MARL: each agent's gradient points it in one direction, but the other agent is simultaneously moving, creating a perpetual chase.
`,
