...
The first layer is the **input layer**. It simply accepts the raw input vector $\\mathbf{x}$ (for example, a state representation in RL). The final layer is the **output layer**, which produces the network's prediction -- perhaps a value estimate for each action. Every layer in between is called a **hidden layer**, because its intermediate representations are not directly exposed to the user.
...
where $\\theta_k$ denotes the parameters of layer $k$, and $\\theta = \\bigcup_k \\theta_k$ is the full parameter set. Two key architectural choices govern the network's capacity: the **depth** (number of layers) and the **width** (number of units per layer, also called the hidden dimension). More layers and wider layers generally allow the network to represent more complex functions, but they also require more data and computation to train.

Inside each layer, the computation is surprisingly simple. Consider a single **neural unit** in layer $k$. It receives the output of the previous layer as input, computes a weighted sum using a weight vector $\\mathbf{w}$ and a scalar bias $b$, and then applies a non-linear **activation function** $g$:

$$f_{k,u}(\\mathbf{x}; \\theta_u) = g(\\mathbf{w}^T \\mathbf{x} + b)$$

Stacking many such units side by side forms a layer, and stacking layers forms the network. In vectorised form, a full layer computes $f_k(\\mathbf{x}_{k-1}; \\theta_k) = g(W_k^T \\mathbf{x}_{k-1} + \\mathbf{b}_k)$, where $W_k$ is a weight matrix and $\\mathbf{b}_k$ is a bias vector. This matrix-vector formulation makes layers efficient to compute on modern hardware, especially GPUs.
`,
...
The theorem states that a feedforward neural network with just a single hidden layer and a sufficient number of hidden units can approximate any continuous function on a closed, bounded subset of $\\mathbb{R}^n$ to arbitrary accuracy. In other words, if the function you want to learn is continuous, there always exists a (possibly very wide) single-hidden-layer network that gets as close to it as you like.
...
- A single neural unit computes $g(\\mathbf{w}^T \\mathbf{x} + b)$, and a full layer is the vectorised version: $g(W^T \\mathbf{x} + \\mathbf{b})$.
