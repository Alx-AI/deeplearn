...
A crucial insight is that the set of joint-action values $\\{Q_1(s, \\cdot), \\ldots, Q_n(s, \\cdot)\\}$ at a given state $s$ can be viewed as the reward matrices of a **normal-form game** $\\Gamma_s$. If we have two agents with three actions each, the game $\\Gamma_s$ for agent $i$ looks like:
...
2. **Select an action** using $\\epsilon$-greedy exploration: with probability $\\epsilon$ choose randomly, otherwise solve the stage game $\\Gamma_{s_t}$ to obtain equilibrium policies $(\\pi_1, \\ldots, \\pi_n)$ and sample $a_i$ from $\\pi_i$.
3. **Observe** the joint action $a_t = (a^t_1, \\ldots, a^t_n)$, each agent's reward $r^t_j$, and the next state $s_{t+1}$.
4. **Update** Q-values for every agent $j$ using a TD rule:

$$Q_j(s_t, a_t) \\leftarrow Q_j(s_t, a_t) + \\alpha \\bigl[r^t_j + \\gamma \\, \\text{Value}_j(\\Gamma_{s_{t+1}}) - Q_j(s_t, a_t)\\bigr]$$

The term $\\text{Value}_j(\\Gamma_{s'})$ is the expected return for agent $j$ under the equilibrium solution of the stage game at the next state $s'$. This mirrors how the max operator works in single-agent Q-learning, but replaces the single-agent "max" with a game-theoretic solver.

Different JAL-GT algorithms arise from plugging in **different solution concepts** for the $\\text{Value}_j$ operator. Minimax gives us Minimax-Q. Nash equilibrium gives us Nash-Q. Correlated equilibrium gives us Correlated-Q. Each choice leads to different convergence properties and applicability conditions.
...
**Full action observability.** Each agent must observe the actions chosen by **all** other agents after every time step. This is essential because the Q-value update requires the full joint action $a_t = (a^t_1, \\ldots, a^t_n)$. Without it, the agent cannot index into the correct entry of its Q-table. In many practical scenarios -- robots in the same warehouse, players in a board game -- this assumption holds naturally. In others -- autonomous vehicles on separate roads, distributed sensor networks -- it may not.
...
Beyond observability, there is the subtle problem of **equilibrium selection**. A game can have multiple equilibria that yield different expected returns. When agents use the $\\text{Value}_j$ operator to compute update targets, they all need to agree on **which** equilibrium to use. If agent 1 computes targets using one Nash equilibrium while agent 2 uses a different one, the Q-values may not converge properly.
...
- Joint-action learning (JAL) extends temporal-difference learning to multi-agent settings by learning Q-values over joint actions $Q_i(s, a_1, \\ldots, a_n)$.
...
- JAL-GT algorithms update Q-values using TD learning where the "max" operator is replaced by a game-theoretic $\\text{Value}_j$ solver.
