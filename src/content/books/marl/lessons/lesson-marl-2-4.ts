...
V^pi(s) = E_pi[G_t | s_t = s] = E_pi[r_t + gamma * r_{t+1} + gamma^2 * r_{t+2} + ... | s_t = s]
...
Q^pi(s, a) = sum_{s' in S} T(s' | s, a) * [R(s, a, s') + gamma * V^pi(s')]
...
This convenience is why many RL algorithms -- including Q-learning and Sarsa, which we will encounter later -- learn Q-functions rather than V-functions. Once you have a good estimate of Q*, the optimal policy is just argmax_a Q*(s, a), no model of the environment needed. The action-value function is the practical workhorse of model-free RL.

As with V^pi, the action-value Q^pi(s, a) = 0 for terminal states s in S-bar. Both V^pi and Q^pi are uniquely determined by the policy pi and the MDP dynamics.
`,
    ...
    },
    {
      ...
      content: `
The recursive structure of the discounted return -- G_t = r_t + gamma * G_{t+1} -- gives rise to the most important equations in RL: the **Bellman equations**.
...
This equation holds simultaneously for every state s in S. For a finite MDP with m = |S| states, it defines a system of m linear equations in m unknowns (the values V^pi(s_1), ..., V^pi(s_m)). Since everything else -- pi, T, R, gamma -- is known, this system has a unique solution. One could solve it by Gaussian elimination in O(m^3) time, though iterative methods are more common in practice.

Similarly, the **Bellman expectation equation** for Q^pi is:

Q^pi(s, a) = sum_{s' in S} T(s' | s, a) * [R(s, a, s') + gamma * sum_{a' in A} pi(a' | s') * Q^pi(s', a')]
...
Q*(s, a) = sum_{s' in S} T(s' | s, a) * [R(s, a, s') + gamma * max_{a' in A} Q*(s', a')]
...
- V^pi and Q^pi are related: V^pi(s) = sum over actions of pi(a|s) * Q^pi(s,a), and Q^pi can be expanded using V^pi and the transition model.
