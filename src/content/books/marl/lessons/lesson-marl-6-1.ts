...
$$M_{s,i}(a) = \\sum_{s'} T(s' \\mid s, a) \\bigl[R_i(s, a, s') + \\gamma \\, V_i(s')\\bigr]$$
...
$$V_i(s) \\leftarrow \\text{Value}_i(M_{s,1}, M_{s,2})$$

Here $\\text{Value}_i$ denotes the unique minimax expected return for agent $i$. In a zero-sum game, this can be computed exactly via a **linear program** (the same LP we met in Section 4.3.1). Notice the structural similarity to MDP value iteration, which uses $V(s) \\leftarrow \\max_a \\bigl[\\sum_{s'} T(s'\\mid s,a)(R + \\gamma\\, V(s'))\\bigr]$. The only difference is that the max-operator is replaced by the $\\text{Value}_i$**-operator**, which solves a two-player game rather than a single-agent optimization.

In fact, if you set $n = 1$ (a single agent), the stochastic game reduces to an MDP, and $\\text{Value}_i$ collapses to a simple max over actions -- recovering standard value iteration exactly.
`,
...
A mapping $f: X \\to X$ on a normed vector space is a $\\gamma$**-contraction** if for all $x, y \\in X$:

$$\\|f(x) - f(y)\\| \\le \\gamma \\, \\|x - y\\|$$

where $\\gamma \\in [0, 1)$. The **Banach fixed-point theorem** then guarantees that repeatedly applying $f$ from any starting point converges to a unique fixed point $x^*$ satisfying $f(x^*) = x^*$.

Shapley showed that the value-update operator satisfies this contraction property under the max-norm $\\|x\\|_\\infty = \\max_i |x_i|$. Therefore, no matter how we initialize $V_i(s)$, the iterative sweeps converge to unique optimal values $V^*_i(s)$ for every agent $i$ and state $s$. These fixed-point values satisfy the **Bellman-like optimality equations**:

$$V^*_i(s) = \\text{Value}_i(M^*_{s,1}, \\ldots, M^*_{s,n})$$
$$M^*_{s,i}(a) = \\sum_{s'} T(s'\\mid s,a)\\bigl[R_i(s,a,s') + \\gamma \\, V^*_i(s')\\bigr]$$

Once we have $V^*_i$, we can extract the **minimax joint policy** of the stochastic game. For each state $s$, we solve the stage game given by $M^*_{s,1}, \\ldots, M^*_{s,n}$ to get equilibrium policies $\\pi_i(a_i \\mid s)$. A key observation: just like optimal policies in MDPs, the resulting minimax policies depend only on the **current state** -- not on the full history of states and actions. This stationarity property is powerful: it means agents need only observe the present state to act optimally (in the minimax sense).
