...
**Step 3 -- Grow populations via oracle.** For each agent $i$, train a new **best-response policy** $\\pi'_i$ against the distribution $\\delta_{-i}^k$ over other agents' populations. This uses single-agent RL in game $G$, where opponent policies are sampled from $\\delta_{-i}^k$ each episode. Add $\\pi'_i$ to agent $i$'s population: $\\Pi_i^{k+1} = \\Pi_i^k \\cup \\{\\pi'_i\\}$.
...
PFSP computes a distribution over opponents weighted by difficulty: $\\delta_i^k(\\pi) \\propto f(\\Pr[\\pi'_i \\text{ beats } \\pi])$. The default weighting $f_{\\text{hard}}(x) = (1-x)^p$ focuses on the hardest opponents; $f_{\\text{var}}(x) = x(1-x)$ focuses on opponents of similar skill.
