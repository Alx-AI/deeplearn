...
$$V_{k+1}(s) \\leftarrow \\sum_{a \\in \\mathcal{A}} \\pi(a \\mid s) \\sum_{s' \\in \\mathcal{S}} T(s' \\mid s, a) \\cdot [R(s, a, s') + \\gamma \\cdot V_k(s')]$$
...
$$\\pi'(s) = \\arg\\max_{a \\in \\mathcal{A}} \\sum_{s' \\in \\mathcal{S}} T(s' \\mid s, a) \\cdot [R(s, a, s') + \\gamma \\cdot V^{\\pi}(s')]$$

Equivalently, using the action-value notation: $\\pi'(s) = \\arg\\max_{a \\in \\mathcal{A}} Q^{\\pi}(s, a)$. The new policy $\\pi'$ selects the single best action in each state according to the current value estimates.

The **policy improvement theorem** guarantees that if the greedy policy $\\pi'$ improves on $\\pi$ in at least one state (and is no worse in any state), then $\\pi'$ is a strictly better policy overall: $V^{\\pi'}(s) \\geq V^{\\pi}(s)$ for all states $s$. This is a remarkable result -- local greedy improvements translate into global improvements in expected return.

**Policy iteration** alternates between policy evaluation and policy improvement:

$$\\pi_0 \\to V^{\\pi_0} \\to \\pi_1 \\to V^{\\pi_1} \\to \\pi_2 \\to \\ldots \\to V^* \\to \\pi^*$$
...
In the Mars Rover example, running policy iteration converges to the optimal policy $\\pi^*$ that chooses "left" in the Start state and "right" in Site A and Site B, achieving $V^*(\\text{Start}) = 4.1$.

Because the number of deterministic policies in a finite MDP is finite ($|\\mathcal{A}|^{|\\mathcal{S}|}$), and each policy improvement step either produces a strictly better policy or terminates, policy iteration is guaranteed to converge in a finite number of iterations. In practice, convergence is typically very fast, often requiring far fewer iterations than the total number of possible policies.
`,
...
$$V_{k+1}(s) \\leftarrow \\max_{a \\in \\mathcal{A}} \\sum_{s' \\in \\mathcal{S}} T(s' \\mid s, a) \\cdot [R(s, a, s') + \\gamma \\cdot V_k(s')]$$
...
$$\\pi^*(s) = \\arg\\max_{a \\in \\mathcal{A}} \\sum_{s' \\in \\mathcal{S}} T(s' \\mid s, a) \\cdot [R(s, a, s') + \\gamma \\cdot V^*(s')]$$
...
For the Mars Rover problem with $\\gamma = 0.95$, value iteration converges to $V^*(\\text{Start}) = 4.1$, $V^*(\\text{Site A}) = 6.2$, and $V^*(\\text{Site B}) = 10$ -- the same optimal values obtained by policy iteration.
