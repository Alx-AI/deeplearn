...
Formally, a joint policy $\\pi_c(a)$ over joint actions $a$ is a **correlated equilibrium** if for every agent $i$ and every **action modifier** $\\xi_i: A_i \\to A_i$:

$$\\sum_a \\pi_c(a) \\cdot R_i(\\xi_i(a_i), a_{-i}) \\leq \\sum_a \\pi_c(a) \\cdot R_i(a)$$
...
The set of correlated equilibria **contains** all Nash equilibria as a special case. A Nash equilibrium is simply a correlated equilibrium where $\\pi_c$ factors into independent policies: $\\pi_c(a) = \\prod_i \\pi_i(a_i)$. In a Nash equilibrium, knowing your own action gives no information about others' actions because the policies are independent. Correlated equilibrium allows for richer dependencies.
...
To verify this is a correlated equilibrium, suppose agent $i$ receives recommendation L. Given $\\pi_c$, agent $i$ infers that agent $j$ will play S with probability 0.5 and L with probability 0.5 (since of the two joint actions containing $a_i = L$, one has $a_j = S$ and one has $a_j = L$). Agent $i$'s expected return from following L is: $2 \\times 0.5 + 6 \\times 0.5 = 4$. If $i$ deviates to S instead, the expected return is: $0 \\times 0.5 + 7 \\times 0.5 = 3.5$. Since $4 > 3.5$, agent $i$ has no incentive to deviate. A similar argument holds when the recommendation is S. So the correlation device achieves something impossible under independent randomization.
...
- $x_a \\geq 0$ for all $a$, and $\\sum_a x_a = 1$

For CCE, the constraints simplify -- we only need that no agent gains from switching to any constant action. The LP for CE has $n \\cdot k^2$ deviation constraints ($n$ agents, $k$ actions each), while CCE has only $n \\cdot k$ constraints. However, both LPs have $k^n$ variables (one per joint action), which grows exponentially in the number of agents.
...
- The inclusion hierarchy is: $\\text{NE} \\subseteq \\text{CE} \\subseteq \\text{CCE}$.
