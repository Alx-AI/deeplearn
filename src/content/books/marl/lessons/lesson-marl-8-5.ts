...
Each agent i learns a utility function Q(h_i, a_i ; theta_i) that depends only on its own observation history and action. Agents select actions greedily with respect to their individual utilities: a_i = argmax Q(h_i, a_i ; theta_i). The critical question: does greedy action selection by each agent individually produce the same result as greedy selection over the full joint-action space?
...
a in A*(h, z ; theta) <==> for all i: a_i in A*_i(h_i ; theta_i)
...
Q(h^t, z^t, a^t ; theta) = sum_{i in I} Q(h_i^t, a_i^t ; theta_i)
...
VDN's strength is simplicity. Its weakness is expressiveness: many value functions cannot be represented as a simple sum. If agent 1's contribution depends on what agent 2 does (a non-linear interaction), VDN cannot capture it. For example, in a game where Q(A,A) = 10, Q(A,B) = 0, Q(B,A) = 0, Q(B,B) = 10, no additive decomposition Q1(a1) + Q2(a2) can represent this -- setting Q1(A) > Q1(B) to match Q(A,A) > Q(B,A) forces Q(A,B) > Q(B,B), which is wrong.
`,
...
for all i, for all a: partial Q(h, z, a ; theta) / partial Q(h_i, a_i ; theta_i) > 0
...
QMIX implements monotonicity through a **mixing network** f_mix: a feedforward neural network that takes individual utilities as input and outputs the centralized value:

Q(h, z, a ; theta) = f_mix(Q(h_1, a_1 ; theta_1), ..., Q(h_n, a_n ; theta_n) ; theta_mix)

Monotonicity is enforced by requiring all weights in the mixing network to be **non-negative**. The parameters theta_mix of the mixing network are not learned directly -- instead, a **hypernetwork** f_hyper receives the centralized information z as input and outputs the mixing network's weights and biases. The hypernetwork applies an absolute-value activation to weight outputs to ensure positivity. Biases are unconstrained.
...
L(theta) = (1/B) * sum[ (r + gamma * max_a Q(h^{t+1}, z^{t+1}, a ; bar{theta}) - Q(h^t, z^t, a^t ; theta))^2 ]
