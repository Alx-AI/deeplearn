...
**Include baselines and optimal values.** Whenever possible, plot the optimal value $V^*(s_0)$ as a dashed horizontal line on the learning curve. This gives an immediate visual indication of how close the algorithm is to the theoretical optimum. In the Mars Rover example, the optimal value $V^*(\\text{Start}) = 4.1$ (computed via value iteration) serves as this reference. Comparing against known baselines -- such as a random policy or a simpler algorithm -- also helps contextualize the results.
