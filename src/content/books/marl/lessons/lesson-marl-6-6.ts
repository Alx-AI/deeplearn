...
Assume we control agent $i$. Let $\\hat{\\Pi}_j$ be the space of possible models for agent $j$. Each model $\\hat{\\pi}_j$ can choose actions based on the interaction history. Agent $i$ starts with a **prior belief** $\\Pr(\\hat{\\pi}_j \\mid h_0)$ assigning probabilities to each model. After observing agent $j$'s action $a^t_j$ in state $s_t$, agent $i$ updates its belief via **Bayes' rule**:

$$\\Pr(\\hat{\\pi}_j \\mid h_{t+1}) = \\frac{\\hat{\\pi}_j(a^t_j \\mid h_t) \\, \\Pr(\\hat{\\pi}_j \\mid h_t)}{\\sum_{\\pi'_j} \\pi'_j(a^t_j \\mid h_t) \\, \\Pr(\\pi'_j \\mid h_t)}$$
...
For **continuous** model spaces -- where $\\hat{\\Pi}_j$ contains all possible state-conditioned policies -- beliefs can be represented using **Dirichlet distributions**. A Dirichlet over $|A_j|$ actions is parameterized by pseudocounts $(\\beta_1, \\ldots, \\beta_{|A_j|})$. Starting from $\\beta_k = 1$ for all $k$ (a uniform prior), each observation of action $a_j$ in state $s$ simply increments the corresponding pseudocount by 1.

The mean of the Dirichlet is $\\hat{\\pi}_j(a_{j,k} \\mid s) = \\beta_k / (\\sum_l \\beta_l)$, which is exactly the empirical frequency used in fictitious play and JAL-AM. But the Dirichlet carries more information: it also quantifies **how confident** we are. After 3 observations, $\\beta = (2, 1, 1)$ says "probably action 1, but we're quite unsure." After 300 observations, $\\beta = (200, 50, 50)$ says "very likely action 1, and we're confident."
`,
...
Agent 1 starts with a uniform prior: $\\Pr(\\text{Coop}) = 0.5$, $\\Pr(\\text{Grim}) = 0.5$. Now, which first action should agent 1 choose?
...
**If agent 1 defects first and agent 2 is Coop**: Agent 2 continues to cooperate (it always does). Agent 1 immediately learns agent 2 is Coop (because Grim would retaliate), and can exploit by defecting for the remaining steps. Total return $= 10 \\times 0 = 0$.

**If agent 1 defects first and agent 2 is Grim**: Agent 2 switches to permanent defection. Agent 1 learns agent 2 is Grim, but now both are stuck defecting. Total return $= 0 + 9 \\times (-3) = -27$.
...
$$VI_i(a_i \\mid h) = \\sum_{\\hat{\\pi}_{-i}} \\Pr(\\hat{\\pi}_{-i} \\mid h) \\sum_{a_{-i}} Q_i(h, (a_i, a_{-i})) \\prod_{j \\neq i} \\hat{\\pi}_j(a_j \\mid h)$$

$$Q_i(h, a) = \\sum_{s'} T(s'\\mid s(h), a) \\bigl[R_i(s(h), a, s') + \\gamma \\max_{a'_i} VI_i(a'_i \\mid (h, a, s'))\\bigr]$$

The crucial difference from standard Bellman equations: $Q_i$ extends the history $h$ to $(h, a, s')$ and recurses back into $VI$ with the updated history. Since $VI$ uses the **posterior** belief $\\Pr(\\hat{\\pi}_{-i} \\mid h)$, this means $VI$ accounts for how beliefs will change in the future under different histories. The recursion depth determines how far ahead VI evaluates the informational impact.

In our Prisoner's Dilemma example with uniform priors, $\\gamma = 1$, and recursion depth 10:

$$VI_1(\\text{Cooperate}) = -9$$
$$VI_1(\\text{Defect}) = -13.5$$
...
This is a key result: VI can produce **cooperation** in the Prisoner's Dilemma -- not through altruism, but through rational cost-benefit analysis of information acquisition. The choice of **prior belief** matters significantly. With $\\Pr(\\text{Coop}) = 0.8$ instead of $0.5$, the VI values flip and both agents defect initially -- the expected cost of provoking Grim is lower when Grim is less likely. This sensitivity to priors connects to the rational learning literature in game theory (Kalai and Lehrer 1993).
`,
