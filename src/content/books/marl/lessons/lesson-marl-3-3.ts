...
2. A finite set of **states** S, with a subset of **terminal states** S_bar that end the game.
3. For each agent i, a finite set of **actions** A_i, with joint action space A = A_1 x ... x A_n.
4. For each agent i, a **reward function** R_i : S x A x S -> R.
5. A **state transition function** T : S x A x S -> [0,1] giving the probability T(s' | s, a) of transitioning to state s' given current state s and joint action a.
6. An **initial state distribution** mu over S (with zero probability on terminal states).

The game starts in an initial state s_0 sampled from mu. At each time step t, every agent observes the current state s_t and independently selects an action. The resulting joint action a_t = (a_1^t, ..., a_n^t) determines both the rewards and the next state s_{t+1}, sampled from T(s_{t+1} | s_t, a_t). The game continues until a terminal state is reached, a maximum horizon T is exceeded, or indefinitely in non-terminating settings.`,
...
Pr(s_{t+1}, r^t | s_t, a_t, s_{t-1}, a_{t-1}, ..., s_0, a_0) = Pr(s_{t+1}, r^t | s_t, a_t)
