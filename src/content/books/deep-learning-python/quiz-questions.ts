...
export const quizQuestions: QuizQuestion[] = [
  ...
  },
  {
    ...
    type: 'multiple-choice',
    options: ['$28 \\times 28$ grayscale images kept as 2D arrays', 'Flattened vectors of 784 values normalized to $[0, 1]$', '$32 \\times 32$ RGB images', 'One-hot encoded vectors of length 10'],
    correctAnswer: 'Flattened vectors of 784 values normalized to [0, 1]',
    explanation: 'MNIST images are $28 \\times 28$ pixels. For a simple dense network, they are reshaped into flat vectors of $784$ ($28 \\times 28$) values and normalized from $[0, 255]$ to $[0, 1]$ by dividing by 255.',
  ...
  },
  {
    id: 'q-1.6-2',
    lessonId: '1.6',
    question: 'A batch of 32 color images of size $256 \\times 256$ would be stored as a rank-4 tensor with shape (32, 256, 256, 3).',
    type: 'true-false',
    options: ['True', 'False'],
    correctAnswer: 'True',
  ...
  },
  {
    ...
    explanation: 'A dense layer computes $\\text{output} = \\text{activation}(\\text{dot}(\\text{input}, W) + b)$. The weight matrix $W$ and bias vector $b$ are the learnable parameters updated during training.',
  ...
  },
  {
    ...
    type: 'multiple-choice',
    options: ['Edges are useful for digit classification', 'A rare word appearing once in a negative review predicts negative sentiment', '$28 \\times 28$ pixel images represent digits', 'Cats and dogs have different shapes'],
  ...
  },
  {
    ...
    explanation: 'pool_size=2 halves each spatial dimension ($16 \\to 8$). The depth/channels (32) are unchanged. MaxPooling downsamples spatially while preserving feature depth.',
  ...
  },
  {
    ...
    options: ['True', 'False'],
    correctAnswer: 'False',
    explanation: 'Fine-tuning requires a much lower learning rate (typically $10\\text{-}100\\times$ smaller) to make small adjustments without destroying the valuable pretrained weights.',
  ...
  },
  {
    id: 'q-4.11-2',
    lessonId: '4.11',
    question: 'SeparableConv2D = depthwise convolution + _____ convolution ($1 \\times 1$, mixing channels).',
    type: 'fill-blank',
    correctAnswer: 'pointwise',
    explanation: 'Depthwise handles spatial filtering per channel independently; pointwise ($1 \\times 1$ conv) handles cross-channel mixing. Together they achieve similar results to standard convolution with far fewer parameters.',
  ...
  },
  {
    ...
    explanation: 'Standard convolutions jointly learn spatial and channel patterns. Separable convolutions factor this into two cheaper operations, often using $8\\text{-}12\\times$ fewer parameters.',
  ...
  },
  {
    ...
    explanation: 'The Embedding layer stores a $10{,}000 \\times 128$ weight matrix. Each token ID indexes one row, retrieving a learned 128-dimensional vector representation.',
  ...
  },
  {
    ...
    explanation: 'LoRA freezes the original model and injects small trainable low-rank decomposition matrices. This reduces trainable parameters by $\\sim 10{,}000\\times$ while achieving comparable performance.',
  ...
  },
  {
    ...
    explanation: 'Float16 uses half the memory of float32 and modern GPUs have dedicated hardware (Tensor Cores) that processes float16 at $2\\text{-}8\\times$ the speed of float32.',
  ...
  },
  {
    ...
    explanation: 'INT8 quantization is a deployment optimization that shrinks model size by $4\\times$ and speeds up inference, with minimal accuracy loss for most models.',
